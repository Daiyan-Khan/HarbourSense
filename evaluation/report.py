"""Generate the portfolio evaluation page from measured JSON results."""
import json
...
def mean_sd(value):
    return f"{value['mean']:.2f} ± {value['sampleStdDev']:.2f}"


def generate():
    results = ROOT / "evaluation/results"
    anomaly = json.loads((results / "anomaly-results.json").read_text())
    routing = json.loads((results / "routing-results.json").read_text())
    forest, threshold = anomaly["isolationForest"], anomaly["threshold"]
    weighted, bfs = routing["summaries"]["weighted"], routing["summaries"]["bfs"]
    provenance = anomaly["provenance"]
    text = f"""# HarbourSense evaluation evidence
...
Measured on {provenance['generatedAt'][:10]}. Source fingerprints, exact package versions, wall-clock execution durations, and machine information are in the result JSON files. Regenerate this page with `python evaluation/report.py`; do not edit result numbers by hand.
...
Ground truth labels an injected fault from its first ramp sample, including samples whose values remain inside healthy ranges. There are **{anomaly['classBalance']['samples']} samples: {anomaly['classBalance']['healthy']} healthy and {anomaly['classBalance']['faulty']} faulty**, across 15 sequences and 10 fault episodes. The class balance is an experimental choice, not an estimated port failure rate.

| Metric | Existing IsolationForest | Fixed range thresholds |
| --- | ---: | ---: |
| Precision | {percent(forest['precision'])} | {percent(threshold['precision'])} |
| Recall | {percent(forest['recall'])} | {percent(threshold['recall'])} |
| False-alarm rate | {percent(forest['falseAlarmRate'])} | {percent(threshold['falseAlarmRate'])} |
| True positives / false positives | {forest['truePositive']} / {forest['falsePositive']} | {threshold['truePositive']} / {threshold['falsePositive']} |
| False negatives / true negatives | {forest['falseNegative']} / {forest['trueNegative']} | {threshold['falseNegative']} / {threshold['trueNegative']} |
| Median first-detection delay | {forest['medianDetectionDelaySecondsAmongDetected']:.2f} simulated s | {threshold['medianDetectionDelaySecondsAmongDetected']:.2f} simulated s |
| P95 first-detection delay | {forest['p95DetectionDelaySecondsAmongDetected']:.2f} simulated s | {threshold['p95DetectionDelaySecondsAmongDetected']:.2f} simulated s |
| Undetected fault episodes | {forest['missedEpisodes']} / 10 | {threshold['missedEpisodes']} / 10 |
...
| --- | ---: | ---: |
| Completed deliveries / 60 | {mean_sd(weighted['completed'])} | {mean_sd(bfs['completed'])} |
| Unfinished deliveries / 60 | {mean_sd(weighted['unfinished'])} | {mean_sd(bfs['unfinished'])} |
| Failed deliveries / 60 | {mean_sd(weighted['failed'])} | {mean_sd(bfs['failed'])} |
| Completed / simulated minute | {mean_sd(weighted['throughputPerSimulatedMinute'])} | {mean_sd(bfs['throughputPerSimulatedMinute'])} |
| Median delivery time, simulated s | {mean_sd(weighted['medianDeliverySecondsAmongCompleted'])} | {mean_sd(bfs['medianDeliverySecondsAmongCompleted'])} |
| P95 delivery time, simulated s | {mean_sd(weighted['p95DeliverySecondsAmongCompleted'])} | {mean_sd(bfs['p95DeliverySecondsAmongCompleted'])} |
| Changes to remaining route | {mean_sd(weighted['routeChanges'])} | {mean_sd(bfs['routeChanges'])} |
...
- OS: {provenance['hardware']['os']}; architecture: {provenance['hardware']['architecture']}.
- Processor: {provenance['hardware']['processor']}; logical CPUs: {provenance['hardware']['logicalCpuCount']}.
- Python {provenance['python']}; numpy {provenance['packages']['numpy']}; scikit-learn {provenance['packages']['scikit-learn']}; scipy {provenance['packages']['scipy']}.
...
"""
    ...
    print("Generated docs/evaluation.md from measured result JSON.")
